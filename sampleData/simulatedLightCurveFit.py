...
import os
import oscaar
from oscaar import IO, transitModel

## Run parameters: 
plotFit = True		## Plot light curve fit
animatePB = False 	## Plot each prayer-bead iteration

dataBank = IO.load(os.path.join(os.path.dirname(__file__),'../sampleOutput/oscaarDataBase.pkl'))
...
def fitfunc(p,t=t,P=P,gamma1=gamma1,gamma2=gamma2,e=e,longPericenter=longPericenter):
    ...
    if p[2] >= 0 and p[2] <= 180:
        ...
        F = transitModel.occultquad(times,modelParams)
        return F
