...
import oscaar
from oscaar import transitModel, fitting
...
if loadvals == False:
    ...
    y = transitModel.occultquad(x,modelParams) + np.random.normal(0,1,x.shape)*noiseFactor
    ...
    def occult4params(t,params):
        ...
        p,ap,i,t0 = params
        return transitModel.occultquad(t,[p,ap,2.2,i,0.23,0.3,0.0,0.0,t0])
    ...
    beta = fitting.optimizeBeta(x,y,sigma_y,inputParams,occult4params,beta,idealAcceptanceRate=0.30)
    ...
    bestp, allparams, acceptanceRate = fitting.mcmc(x,y,sigma_y,inputParams,occult4params,Nsteps,beta,saveInterval,verbose=True)
...
def histplot(parameter,axis,title,bestFitParameter):
    ...
    plus,minus = fitting.get_uncertainties(postburn,bestFitParameter)   ## Calculate uncertainties on best fit parameter
